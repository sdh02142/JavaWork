...
public class BoardController {
   ...
   public static void boardSelectByAll() {
	...
	public static void boardSelectByNo(int boardNo) {
		try {
		     BoardDTO bDTO = boardService.boardSelectByNo(boardNo);
		     SuccessView.selectByNoPrint(bDTO);
		   }catch (SearchWrongException e) {
			  FailView.errorMessage(e.getMessage());
		  }
	}
	...
	public static void replyInsert(ReplyDTO replyDTO) {
		try {
			 ...
			 SuccessView.messagePrint("댓글 등록되었습니다.");
			}catch (DMLException e) {
				FailView.errorMessage(e.getMessage());
			}
	}
	...
	public static void replySelectByParentNo(int boardNo) {
		try {
			 SuccessView.selectReplyPrint(boardService.replySelectByParentNo(boardNo));
			}catch (SearchWrongException e) {
				FailView.errorMessage(e.getMessage());
			}
	}
